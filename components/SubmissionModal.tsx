...
interface SubmissionModalProps {
  ...
  onDelete: (dateStr: string, name: string) => Promise<void> | void; // ✅ NEW
}
...
const SubmissionModal: React.FC<SubmissionModalProps> = ({
  ...
  const handleDelete = async (s: AvailabilitySubmission) => {
    const key = `${s.date}|${s.name}`;
    setDeletingKey(key);

    try {
      await onDelete(s.date, s.name);
    } finally {
      setDeletingKey(null);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-md overflow-y-auto">
      <div className="bg-[#1a1a1a] border-2 border-[#b08d57] w-full max-w-md p-8 rounded-lg shadow-[0_0_80px_rgba(0,0,0,1)] relative my-8">
        ...
        <h2 className="text-3xl font-cinzel text-[#b08d57] mb-2 text-center tracking-wide">
          DECLARE PRESENCE
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-[10px] font-medieval text-stone-500 uppercase mb-2 tracking-widest">
              Identify Traveler
            </label>
            <select
              value={name}
              ...
              style={{ color: 'black' }}
            >
              <option value="" className="text-black bg-[#d4c3a1]">
                Choose Companion
              </option>
              {CHARACTER_NAMES.map(n => (
                <option key={n} value={n} className="text-black bg-[#d4c3a1]">
                  {n}
                </option>
              ))}
            </select>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                {TIMEZONES.map(tz => (
                  <option key={tz} value={tz} className="text-black bg-[#d4c3a1]">
                    {tz}
                  </option>
                ))}
              </select>
              ...
              <label className="flex items-center gap-2 cursor-pointer group py-3 px-1">
                ...
                <div
                  className={`w-5 h-5 border flex items-center justify-center transition-all ${
                    isAllDay ? 'bg-[#b08d57] border-[#b08d57]' : 'bg-transparent border-stone-700'
                  }`}
                >
                  {isAllDay && <span className="text-stone-950 text-[10px]">✓</span>}
                </div>
                <span
                  className={`text-[10px] font-medieval uppercase tracking-widest ${
                    isAllDay ? 'text-[#b08d57]' : 'text-stone-500'
                  }`}
                >
                  All Day
                </span>
          ...
          {!isAllDay && (
            <div className="grid grid-cols-2 gap-4 pt-2 animate-in fade-in slide-in-from-top-2">
              <div>
                <label className="block text-[10px] font-medieval text-stone-500 uppercase mb-2 tracking-widest text-center">
                  Arrival
                </label>
                <input
                  type="time"
        ...
        {existingSubmissions.length > 0 && (
          <div className="mt-8 pt-6 border-t border-stone-800/50">
            <h3 className="text-[10px] font-medieval text-stone-600 uppercase mb-4 tracking-widest">
              Party Already Committed
            </h3>
              ...
              {existingSubmissions.map(s => {
                ...
                return (
                  <div key={key} className="group relative">
                    <div
                      className={`text-[10px] px-3 py-2 rounded-sm border font-cinzel flex flex-col items-center ${
                        s.isAllDay
                    ...
                    >
                      ...
                      <button
                        ...
                        disabled={isDeleting}
                        className="mt-2 text-[9px] uppercase tracking-widest font-medieval text-stone-400 hover:text-rose-400 disabled:opacity-50"
            ...
            <p className="mt-3 text-[9px] text-stone-600 font-medieval uppercase tracking-widest">
              Tip: Remove deletes only that character’s entry for this date.
            </p>
          </div>
        )}
      </div>
